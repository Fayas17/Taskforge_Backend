...
def test_jwt_none_algorithm(client):
    ...
    try:
        ...
        malicious_token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJub25lIn0.eyJzdWIiOiJtYWxpY2lvdXNAZXhhbXBsZS5jb20iLCJ0eXBlIjoiYWNjZXNzIn0."
...
def test_jwt_invalid_signature(client):
    ...
    tampered_payload_b64 = base64.urlsafe_b64encode(json.dumps(payload_dec).encode()).decode().rstrip("=")
    ...
    response = client.get("/auth/me/")  
    assert response.status_code == 401, "Backend must reject forged payloads with invalid signatures"
